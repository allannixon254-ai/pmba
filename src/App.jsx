...
export default function StudyPlan() {
  ...
  const [lastActiveId,    setLastActiveId]    = useState(null); // O visor fixo que segura a tela pausada
  ...
  useEffect(() => {
    ...
    try { const t = localStorage.getItem("pmba-timers");   if (t) setSavedTotals(JSON.parse(t));   } catch(e) {}
    try { const s = localStorage.getItem("pmba-sessions"); if (s) setSessionTimers(JSON.parse(s)); } catch(e) {}
    setLoaded(true);
  }, []);
  ...
  // ── FUNÇÕES DO CRONÔMETRO CORRIGIDAS
  const saveTotals   = (t) => { try { localStorage.setItem("pmba-timers", JSON.stringify(t)); } catch(e) {} };
  const saveSessions = (s) => { try { localStorage.setItem("pmba-sessions", JSON.stringify(s)); } catch(e) {} };

  const getSessionSeconds = (id) => {
    const base = sessionTimers[id] || 0;
    if (activeTimerInfo?.id === id) return base + Math.floor((Date.now() - activeTimerInfo.startedAt) / 1000);
    return base;
  };

  const getSubjectTotalSeconds = (id) => {
    return (savedTotals[id] || 0) + getSessionSeconds(id);
  };

  const getTotalSecs = () => {
    const totalSaved = Object.values(savedTotals).reduce((a,b) => a + b, 0);
    const totalSessions = Object.values(sessionTimers).reduce((a,b) => a + b, 0);
    const activeSecs = activeTimerInfo ? Math.floor((Date.now() - activeTimerInfo.startedAt) / 1000) : 0;
    return totalSaved + totalSessions + activeSecs;
  };

  const iniciar = (id) => {
    if (activeTimerInfo && activeTimerInfo.id !== id) pausar(activeTimerInfo.id);
    setActiveTimerInfo({ id, startedAt: Date.now() });
    setLastActiveId(id); // Atualiza o visor visual
  };

  const pausar = (idToPause = activeTimerInfo?.id) => {
    if (!idToPause || activeTimerInfo?.id !== idToPause) return;
    const elapsed = Math.floor((Date.now() - activeTimerInfo.startedAt) / 1000);
    const nt = { ...sessionTimers, [idToPause]: (sessionTimers[idToPause] || 0) + elapsed };
    setSessionTimers(nt); 
    saveSessions(nt);
    setActiveTimerInfo(null);
    // lastActiveId intencionalmente NÃO é limpo aqui para congelar o visor
  };

  const finalizar = (id) => {
    let sessionElapsed = sessionTimers[id] || 0;
    if (activeTimerInfo?.id === id) {
      sessionElapsed += Math.floor((Date.now() - activeTimerInfo.startedAt) / 1000);
      setActiveTimerInfo(null);
    }
    
    if (sessionElapsed > 0) {
      const newTotals = { ...savedTotals, [id]: (savedTotals[id] || 0) + sessionElapsed };
      setSavedTotals(newTotals); 
      saveTotals(newTotals);
      
      const newSessions = { ...sessionTimers };
      delete newSessions[id];
      setSessionTimers(newSessions); 
      saveSessions(newSessions);
    }
    if (lastActiveId === id) setLastActiveId(null);
  };

  const zerar = (id) => {
    if (activeTimerInfo?.id === id) setActiveTimerInfo(null);
    const nt = { ...sessionTimers }; 
    delete nt[id];
    setSessionTimers(nt); 
    saveSessions(nt);
    if (lastActiveId === id) setLastActiveId(null);
          ...
          <div style={{ background:"#1e293b", borderRadius:12, padding:"11px 16px", marginBottom:14, border:"1px solid #334155", display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
            ...
            {todayData.subjects.length === 0 ? (
              ...
              todayData.subjects.map(sub => {
                ...
                const secs = getSessionSeconds(sub.id);
                return (
                  <span
                    key={sub.id}
                    onClick={() => isActive ? pausar(sub.id) : iniciar(sub.id)}
                    style={{
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {/* Aqui está o visor em ação */}
                {lastActiveId ? fmtTimer(getSessionSeconds(lastActiveId)) : "00:00:00"}
              </div>
              <div style={{marginTop:10, fontSize:12, color:"#64748b", minHeight:18}}>
                {lastActiveId
                  ? <span>
                      {activeTimerInfo ? "⏱ Em andamento" : "⏸ Pausado"} — <span style={{color:"#fbbf24"}}>
                        {curriculum.find(s => s.id === lastActiveId)?.name}
                      </span>
                    </span>
            ...
            {/* CARDS DE MATÉRIA INVERTIDOS */}
            {curriculum.map(s => {
              const pc           = PRIORITY[s.priority];
              const isActive     = activeTimerInfo?.id === s.id;
              const secs         = getSessionSeconds(s.id);
              const totalMateria = getSubjectTotalSeconds(s.id);
                  ...
                  <div style={{display:"flex", alignItems:"center", gap:12, flexWrap:"wrap"}}>
                    ...
                    <div style={{flex:1, minWidth:140}}>
                      ...
                      <div style={{
                        ...
                        color: isActive ? "#fbbf24" : isPaused ? "#e2e8f0" : (totalMateria > 0 ? "#94a3b8" : "#1e293b"),
                        transition:"color .3s",
                      }}>
                        {fmtTimer(totalMateria)}
                      </div>
                    </div>
                    ...
                    <div style={{display:"flex", gap:8, flexWrap:"wrap", alignItems:"center"}}>
                      ...
                      {isActive && (
                        <>
                          <Btn onClick={() => pausar(s.id)} color="#d97706">⏸ Pausar</Btn>
                          <Btn onClick={() => finalizar(s.id)} color="#dc2626">⏹ Salvar</Btn>
                        </>
                      )}
                      {isPaused && (
                        <>
                          ...
                          <Btn onClick={() => finalizar(s.id)} color="#dc2626">⏹ Salvar</Btn>
                          <Btn onClick={() => zerar(s.id)} color="#475569">🔄 Recomeçar</Btn>
                        </>
                      )}
                    </div>
                  </div>
              ...
              <div style={{fontSize:11, color:"#64748b", marginTop:3}}>
                ...
                {activeTimerInfo && (
                  <span style={{marginLeft:10, color:"#f59e0b"}}>
                    ⏱ {activeSubjectName} em andamento — {fmtTimer(getSessionSeconds(activeTimerInfo.id))}
                  </span>
                )}
              </div>
            </div>
                  ...
                  {day.subjects.length > 0 && (
                    <div style={{padding:"12px 16px", display:"flex", gap:10, flexWrap:"wrap"}}>
                      {day.subjects.map(sub => {
                        const isActive     = activeTimerInfo?.id === sub.id;
                        const secs         = getSessionSeconds(sub.id);
                        const totalMateria = getSubjectTotalSeconds(sub.id);
                        const isPaused     = secs > 0 && !isActive;
                        ...
                        return (
                          ...
                          >
                            ...
                            <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
                              ...
                              {isActive && (
                                <>
                                  <Btn small onClick={() => pausar(sub.id)} color="#d97706">⏸ Pausar</Btn>
                                  <Btn small onClick={() => finalizar(sub.id)} color="#dc2626">⏹ Salvar</Btn>
                                </>
                              )}
                              {isPaused && (
                                <>
                                  ...
                                  <Btn small onClick={() => finalizar(sub.id)} color="#dc2626">⏹ Salvar</Btn>
                                  <Btn small onClick={() => zerar(sub.id)} color="#475569">🔄 Recomeçar</Btn>
                                </>
                              )}
                            </div>
                          </div>
